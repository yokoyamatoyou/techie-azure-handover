...
ROUTE_ID = "semi_spoken_linebreak_register_shadow_v1"
DEFAULT_OUTPUT_ROOT = (
    PROJECT_ROOT
    / "logs"
    / "shadow_autonomous_20260504-233916"
    / "routes"
    / "semi_spoken_linebreak_register_live_20260506"
)
...
def _prompt(contract: dict[str, Any]) -> str:
    return f"""保存済みsourceだけで、company_introduction のブログ記事を1本書く。

register contract:
意味の節目で改行する。短い一文段落も自然なら使う。全ての文を丁寧な説明文にしない。

stance:
会社は話者ではなく取材対象。読者の手元の業務から始め、相談前に決めることへ静かに着地する。

output:
普通のMarkdown本文。JSON、title/bodyラベル、構成表は出さない。会社の一人称は使わない。
...
def _write_compare(root: Path, report: dict[str, Any]) -> None:
    ...
    lines = [
        "# Route A Visual Compare: semi_spoken_linebreak_register_shadow_v1",
        "",
        ...
        "## Judgment",
        "",
        "This route tests a compact semi-spoken register contract only. Route A remains the frozen reference.",
    ]
...
def _write_failure_note(root: Path, report: dict[str, Any]) -> None:
    lines = [
        "# Failure Note: semi_spoken_linebreak_register_shadow_v1",
        "",
        f"Date: {report['date_jst']}",
        "",
        "## Decision",
        ...
        "## Do Not Repeat",
        "",
        "- Do not add more linebreak/register instructions to rescue this route.",
...
def run(root: Path) -> dict[str, Any]:
    ...
    report: dict[str, Any] = {
        "route_id": ROUTE_ID,
        "date_jst": _now_jst(),
        "mode": "one-source semi-spoken linebreak register live validation",
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Opt-in semi-spoken linebreak register shadow route.")
