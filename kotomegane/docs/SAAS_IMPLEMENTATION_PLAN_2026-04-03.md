...
- `billing_rules / plan_catalog / run_policy` の分離は current plan の対象であり、専用 phase を置く
- OpenAI の live manual / batch 実運用確認は `Final Verification` に含める
- Gemini / Claude の live adapter 実装は current plan の必須完了条件には入れず、後続 package へ分離する
...
- `コトメイク` 手動: 1回 1クレジット
- `コトミガキ` 手動: 1回 1クレジット
- `コトメガネ` 手動: 実行完了時に 1回 2クレジット
- `コトメガネ` batch: 手動クレジットとは別料金
- `コトメガネ` batch 料金単位: 結果反映完了時に 1 単位消費

完了時消費の扱い:

- 手動は `run_session.finished_at` が入り、成功結果が保存された時点を完了として扱う
- batch / scheduled は provider batch の投入時点では消費せず、import が終わって `keyword_result` または error item が保存され、`batch_job.imported_at` が入った時点を完了として扱う
- ユーザー停止、開始前 validation error、provider submit failure、import できない失敗は消費しない
- 将来の実クレジット台帳では `run_id` または `batch_job_id` を idempotency key にして二重消費を防ぐ
...
### Phase 6: Packaging And Runtime Policy Separation

目的:
- deliverables に含めた `billing_rules / plan_catalog / run_policy` の責務分離を完了する
...
実装手順:
1. `plan_catalog` を分離し、手動可否 / batch 可否 / provider ごとの総質問数上限を持たせる
2. `billing_rules` を分離し、`service x mode x provider_count` ごとの課金単位を表現する
3. `run_policy` を分離し、手動表示 / batch 暫定表示 / timeout 後の partial 表示方針を持たせる
4. 現行 `config.py` と app / scheduler に散っている実行制御の owner を整理する
5. user-facing UI に不要な内部課金語を増やさず、内部責務だけを移す

自己テスト:
1. plan / billing / run policy を module 経由で取得できる
2. OpenAI manual / batch / scheduled batch の現行導線が壊れない
3. provider ごとの質問数上限と partial display policy の参照元が分離後も一貫する

完了条件:
- `billing_rules / plan_catalog / run_policy` が phase として完了している
- 既存 runtime 挙動を壊さずに責務分離できている
...
14. OpenAI live manual run 1 回
15. OpenAI live batch submit / refresh / import 1 回
...
## 8.5 Deferred After Current Plan

次は current plan の必須完了条件には入れない。

1. Gemini / Claude の live adapter 実装
2. Batch cancel 導線と高度な監視UI
3. Windows service / 外部 scheduler への移行
4. `実行結果比較` の 2 run 超 / settings diff 統合
5. `query_plan` と variance 指標の report 露出拡張

理由:

- 現在の current plan の主眼は `score / variance / UI / provider差分分離` である
- OpenAI single-provider を維持したまま current plan を完了できる
- 上記は別 package に分けた方が rollback と検証がしやすい
...
- Gemini / Claude live adapter は deliverable に含めない
- ただし provider strategy と adapter 吸収ポイントは壊さない
