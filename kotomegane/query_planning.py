...
from prompt_catalog import build_fallback_expansions_from_templates, build_prompt_taxonomy_records
...
class QueryPlanSpec:
    ...
    prompt_taxonomy_items: list[dict[str, Any]]
    expansion_mode: str
...
class QueryPlanner:
    ...
    def _reuse_previous_plan(
        ...
        *,
        ...
        return QueryPlanSpec(
            ...
            prompt_taxonomy_items=self._build_prompt_taxonomy(
                normalized_query,
                expanded_queries,
                config,
                user_query_short=normalize_text(previous_plan.get("user_query_short") or normalized_query),
    ...
    def _build_fallback_expansions(
        ...
        *,
        ...
        return build_fallback_expansions_from_templates(
            short_query,
            original_query,
    ...
    def _build_prompt_taxonomy(
        ...
        *,
        ...
        return build_prompt_taxonomy_records(
            user_query_raw,
            expanded_queries,
            template_set=config.query_expansion_template_set,
            brand_terms=config.brand_terms,
            user_query_short=user_query_short,
            query_was_shortened=query_was_shortened,
        )
    ...
    def _resolve_query_limits(
        ...
        *,
        ...
        provider = get_provider_option(config.provider)
        provider_total_budget = get_provider_total_question_budget(
            provider.key,
...
def build_execution_plan(query_plans: list[dict[str, Any]], config: AppConfig) -> ExecutionPlan:
    ...
    return ExecutionPlan(
        ...
        total_question_budget=get_provider_total_question_budget(
            config.provider,
            service_key=config.service_key,
            plan_key=config.plan_key,
        ),
