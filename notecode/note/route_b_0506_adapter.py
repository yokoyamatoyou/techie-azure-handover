"""Adapter for evaluating the Desktop 0506 blog pipeline as Route B.

This module keeps the current Route A runtime untouched.  It imports the
0506 workspace only when explicitly called by a shadow/preflight tool.
"""
...
ROUTE_B_0506_ROUTE_ID = "route_b_0506_structured_blog_v1"
DEFAULT_ROUTE_B_0506_ROOT = Path(
    os.getenv("NOTECODE_ROUTE_B_0506_ROOT", r"C:\Users\横山裕明\Desktop\0506")
)


class RouteB0506Error(RuntimeError):
    """Raised when the 0506 Route B candidate cannot be prepared."""


@dataclass(frozen=True)
class RouteBSourceRecord:
    title: str
    content: str
    locator: str
    source_type: str
...
def resolve_route_b_0506_genre(input_contract: Mapping[str, Any]) -> str:
...
def resolve_route_b_0506_narrator(input_contract: Mapping[str, Any]) -> str | None:
    if resolve_route_b_0506_genre(input_contract) == "announcement":
        return "当社"
...
def extract_route_b_source_records(
    input_contract: Mapping[str, Any],
    *,
    max_sources: int = 5,
) -> list[RouteBSourceRecord]:
    records: list[RouteBSourceRecord] = []
...
def route_b_source_snapshot_hash(records: list[RouteBSourceRecord]) -> str:
    payload = [
        {
            "title": record.title,
...
def inspect_route_b_0506_workspace(route_b_root: Path = DEFAULT_ROUTE_B_0506_ROOT) -> dict[str, Any]:
    root = Path(route_b_root)
    required_files = [
        "AGENTS.md",
        "README.md",
        "TASK.md",
...
def run_route_b_0506_candidate(
    ...
    *,
    ...
    runner = modules["BlogPipelineRunner"](client=modules["LocalPipelineClient"](), logger=logger)
    result = runner.run_extracted_sources(
        extracted_sources,
        genre_id=genre_id,
        target_reader=target_reader,
        article_goal=article_goal,
        narrator=resolve_route_b_0506_narrator(input_contract),
    )
    return {
        "route_id": ROUTE_B_0506_ROUTE_ID,
        "route_source_workspace": str(Path(route_b_root)),
...
def _load_route_b_0506_modules(route_b_root: Path) -> dict[str, Any]:
    root = route_b_root.resolve()
    ...
    if existing_app is not None:
        ...
        if app_file and not Path(app_file).resolve().is_relative_to(root):
            raise RouteB0506Error(f"python_module_app_already_loaded_from_other_root:{app_file}")
    root_text = str(root)
    if root_text not in sys.path:
        sys.path.insert(0, root_text)
    route_b_site_packages = root / ".venv" / "Lib" / "site-packages"
    route_b_site_packages_text = str(route_b_site_packages)
    if route_b_site_packages.exists() and route_b_site_packages_text not in sys.path:
        sys.path.insert(1, route_b_site_packages_text)
    try:
        ...
        raise RouteB0506Error(f"route_b_0506_import_failed:{type(exc).__name__}:{exc}") from exc
    return {
