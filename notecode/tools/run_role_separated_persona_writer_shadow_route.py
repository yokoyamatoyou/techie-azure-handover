...
ROUTE_ID = "role_separated_persona_writer_shadow_v1"
DEFAULT_OUTPUT_ROOT = (
    PROJECT_ROOT
    / "logs"
    / "shadow_autonomous_20260504-233916"
    / "routes"
    / "role_separated_persona_writer_live_20260506"
)
...
def _messages(contract: dict[str, Any]) -> list[dict[str, str]]:
    return [
        {
            "role": "system",
            "content": "会社運営ブログの編集者。会社を代弁せず、読者の手元の作業から考える。会社名は取材対象として扱う。",
        },
        {
            "role": "developer",
            "content": "保存済みsourceだけで本文を書く。普通のMarkdown記事にする。JSON、title/bodyラベル、構成表、チェックリストは出さない。会社の一人称は使わない。最後は相談前に決めることへ静かに着地する。",
        },
        {
            "role": "user",
            "content": f"company_introduction のブログ記事を1本書いてください。\n\nsource:\n{anchor_route._source_brief(contract)}",
        },
    ]
...
def _call(messages: list[dict[str, str]], model: str, capability: dict[str, bool]) -> str:
    params: dict[str, Any] = {
        "model": model,
        "messages": messages,
...
def _write_compare(root: Path, report: dict[str, Any]) -> None:
    ...
    lines = [
        "# Route A Visual Compare: role_separated_persona_writer_shadow_v1",
        "",
        ...
        "## Judgment",
        "",
        "This route tests message-role separation only. Route A remains the frozen reference.",
    ]
...
def _write_failure_note(root: Path, report: dict[str, Any]) -> None:
    lines = [
        "# Failure Note: role_separated_persona_writer_shadow_v1",
        "",
        f"Date: {report['date_jst']}",
        "",
        "## Decision",
        ...
        "## Do Not Repeat",
        "",
        "- Do not add more role-separated instruction text to rescue this route.",
...
def run(root: Path) -> dict[str, Any]:
    ...
    base._write_json(root / "writer_messages_preview.json", messages)
    model = _model()
    ...
    report: dict[str, Any] = {
        "route_id": ROUTE_ID,
        "date_jst": _now_jst(),
        "mode": "one-source role-separated persona writer live validation",
    ...
    try:
        ...
        text = _call(messages, model, capability)
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Opt-in role-separated persona writer shadow route.")
