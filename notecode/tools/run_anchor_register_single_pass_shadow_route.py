...
ROUTE_ID = "anchor_register_single_pass_shadow_v1"
DEFAULT_OUTPUT_ROOT = (
    PROJECT_ROOT
    / "logs"
    / "shadow_autonomous_20260504-233916"
    / "routes"
    / "anchor_register_single_pass_live_20260506"
)
...
stance:
会社は話者ではなく取材対象。読者の手元の業務から始め、上の meaning anchors が後半に自然に残るように書く。
...
def _write_compare(root: Path, report: dict[str, Any]) -> None:
    ...
    lines = [
        "# Route A Visual Compare: anchor_register_single_pass_shadow_v1",
        "",
        ...
        "## Judgment",
        "",
        "This route tests single-pass combination of meaning anchors and semi-spoken register only.",
    ]
...
def _write_failure_note(root: Path, report: dict[str, Any]) -> None:
    lines = [
        "# Failure Note: anchor_register_single_pass_shadow_v1",
        "",
        f"Date: {report['date_jst']}",
        "",
        "## Decision",
        ...
        "## Do Not Repeat",
        "",
        "- Do not add more anchors or register instructions to rescue this route.",
...
def run(root: Path) -> dict[str, Any]:
    ...
    report: dict[str, Any] = {
        "route_id": ROUTE_ID,
        "date_jst": _now_jst(),
        "mode": "one-source anchor-register single-pass live validation",
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Opt-in anchor-register single-pass shadow route.")
