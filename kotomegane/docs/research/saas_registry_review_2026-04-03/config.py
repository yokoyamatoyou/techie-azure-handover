...
PROVIDER_CATALOG: tuple[ProviderOption, ...] = (
    ProviderOption(
        ...
        cost_reduction_note="通常実行は prompt caching、まとめ確認は Batch API を優先します。",
    ...
    ProviderOption(
        ...
        status="planned",
        summary="今後の拡張枠",
        note="設定・能力表は用意。API 接続は未実装",
        default_model="gemini-3.1-flash-lite-preview",
        default_total_question_budget=30,
        env_var="GEMINI_API_KEY",
        supports_live_requests=False,
        supports_batch=False,
        supports_prompt_cache=False,
        supports_extended_prompt_cache=False,
        supports_query_planner_reuse=True,
        preferred_batch_mode="none",
        cache_policy_mode="provider_defined",
        expansion_mode="llm_planner",
        max_expansion_queries=4,
        batch_timeout_hours=24,
        partial_display_after_hours=24,
        partial_display_policy="partial_after_timeout_gray_pending",
        cost_reduction_note="provider adapter 実装後に caching / batch 条件を個別反映します。",
...
def get_provider_total_question_budget(provider_key: str) -> int:
    provider = get_provider_option(provider_key)
    return max(1, int(provider.default_total_question_budget or 1))
