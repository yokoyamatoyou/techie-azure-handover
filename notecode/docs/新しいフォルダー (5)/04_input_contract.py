...
_SOURCE_META_PATTERN = re.compile(
    ...
    r"利用規約|個人情報保護方針|会社案内PDF|ダウンロード)",
    re.I,
)
...
def _build_source_grounding_items(normalized: Dict[str, Any]) -> List[Dict[str, str]]:
    ...
    for _, _, _, item in gathered:
        ...
        if not prompt_primary_company_intro and doc_key and doc_counts.get(doc_key, 0) >= 2:
            continue
...
def _derive_focus_bundle(normalized: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if prompt_primary:
        for item in source_grounding_items:
            ...
            if anchor and anchor not in support_points:
                support_points.append(anchor)
    else:
        for candidate in (
...
def _derive_must_cover(normalized: Dict[str, Any]) -> List[str]:
    ...
    if prompt_primary:
        available_buckets = [
            _focus_bucket_anchor(str(item.get("bucket") or "other"))
            for item in list(normalized.get("source_grounding_items", []) or [])
            if isinstance(item, dict)
        ]
        bucket_items = _dedupe_text_list(available_buckets, limit=3)
        return bucket_items or ["事業内容", "強み", "歩み"]
...
def _build_strict_saas_missing_items(contract: Dict[str, Any]) -> List[Dict[str, str]]:
    ...
    if article_type in {"branding", "case_study", "announcement"} and content_goal not in {"", "auto"} and not core_message:
        missing_items.append(
            {
                "field": "core_message",
                "issue_type": "missing",
