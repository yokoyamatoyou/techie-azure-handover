...
/**
 * Apply-incapable identity-binding hardening transaction engine.
 *
 * This module has no CLI, apply option, or commit branch. The reviewed wrapper
 * supplies already hash-verified SQL bytes and an independently confirmed DB
 * target hash. The only successful terminal action is transaction rollback.
 */
...
const SUCCESS_LINE = 'HARDENING_DRY_RUN_PASS target_confirmed=true business_state_confirmed=true identity_rows=0 commit_state=not_committed';
...
async function summary(client) {
  const result = await client.query(`
    /* techie-identity-hardening-dryrun-only:summary */
    SELECT
...
async function runHardeningDryRun({
  ClientClass,
  confirmedTargetHash,
  sqlBytes,
  env = process.env,
  ...
  try {
    ...
    await client.query("SELECT pg_advisory_xact_lock(hashtextextended('techie-identity-hardening-dryrun-only-20260804', 0))");
    ...
    assertUnhardenedState(restored, restoredRows, 'ROLLBACK');
    ...
    output.error(`HARDENING_DRY_RUN_ERROR ${safeErrorCode(error)} commit_state=not_committed`);
    return { ok: false, mode: 'dry-run', committed: false };
  } finally {
