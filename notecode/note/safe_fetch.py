...
DEFAULT_MAX_BYTES = int(os.getenv("NOTECODE_SAFE_FETCH_MAX_BYTES", str(10 * 1024 * 1024)))
DEFAULT_TIMEOUT = float(os.getenv("NOTECODE_SAFE_FETCH_TIMEOUT", "20"))
MAX_REDIRECTS = int(os.getenv("NOTECODE_SAFE_FETCH_MAX_REDIRECTS", "5"))
DEFAULT_USER_AGENT = os.getenv("NOTECODE_SAFE_FETCH_USER_AGENT", "JapaneseWritingMVPBot")
...
class _FixedIPMixin:
    ...
    def _new_conn(self):  # type: ignore[override]
        target_host = self._fixed_ip or getattr(self, "_dns_host", self.host)
...
def safe_fetch_url(
    ...
    *,
    ...
    if response is None or session is None or final_target is None:
    ...
    if content_length:
        ...
        if declared_length > max_bytes:
            response.close()
            session.close()
            raise ContentTooLargeError(final_target.normalized_url, declared_length, max_bytes)
