...
from config import ANALYSIS_MODE_MARKET, AppConfig, get_provider_option
...
def _row_provider_label(row: dict[str, Any], config: AppConfig) -> str:
    provider_key = str(_row_run_config(row).get("provider") or config.provider or "openai")
    return provider_display_label(get_provider_option(provider_key))
...
def _run_has_successful_result(rows: list[dict[str, Any]], run_id: str) -> bool:
    ...
    for row in rows:
        if str(row.get("run_id") or "") != target_run_id:
            continue
        if str(row.get("error_text") or "").strip():
            continue
        payload = parse_json_object(row.get("output_json"))
        if normalize_text(payload.get("answer_snapshot")) == normalize_text("実行エラー。設定またはAPI応答を確認してください。"):
            continue
        return True
    return False
...
def resolve_current_result_run_id(
    ...
    *,
    ...
    if preferred_run_id:
        ...
        if any(float(row.get("run_finished_at") or 0.0) > 0 for row in preferred_rows) and _run_has_successful_result(
            matching_rows,
            preferred_run_id,
        ):
    ...
    for row in matching_rows:
        ...
        finished_at = float(row.get("run_finished_at") or 0.0)
        if finished_at <= 0:
            continue
        if time.time() - finished_at > CURRENT_RESULT_RESTORE_WINDOW_SECONDS:
            continue
        if _run_has_successful_result(matching_rows, run_id):
            return run_id
    return ""
...
def resolve_previous_delta_summary(db: Any, config: AppConfig, filtered_rows: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    for row in filtered_rows:
        ...
        if len(run_ids) >= 2:
            break
    if len(run_ids) < 2:
        return {"available": False, "reason": "前回の保存結果との差はまだありません。"}
    ...
    if not current_plans or not previous_plans:
        return {"available": False, "reason": "前回の保存結果との差に必要な拡張条件がまだ保存されていません。"}
    ...
    if current_scope_id != previous_scope_id:
        return {"available": False, "reason": "比較条件が変わったため前回の保存結果との差なし"}
    ...
    if not current_rows or not previous_rows:
        return {"available": False, "reason": "前回の保存結果との差に必要な結果がまだ揃っていません。"}
        ...
        table_rows.append(
            {
                ...
                "provider_label": _row_provider_label(row, config),
