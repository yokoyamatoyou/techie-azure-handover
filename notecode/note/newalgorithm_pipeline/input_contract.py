...
from note.published_post_inventory import normalize_past_blog_context
...
_INJECTION_PATTERNS = (
    ...
    ("previous_instruction_override", re.compile(r"前の指示を無視", re.I)),
    ("system_role_inline", re.compile(r"\bSYSTEM\s*:", re.I)),
    ("guard_disable", re.compile(r"source\s*grounding\s*guard.*無効", re.I)),
    ("unsupported_price_assertion", re.compile(r"価格を断定", re.I)),
...
_SEMANTIC_ALLOWED_PRONOUN_DEFAULTS: Dict[str, List[str]] = {
    "company_introduction": ["私たち", "当社", "弊社"],
    "product_introduction": ["私たち", "当社", "弊社"],
    "branding": ["私たち", "当社", "弊社"],
    "daily_story": ["私", "わたし"],
    "case_study": ["私たち", "当社", "弊社"],
    "implementation_case": ["私たち", "当社", "弊社"],
}
...
_DAILY_STORY_PROMPT_SURFACE_LABELS: tuple[tuple[str, re.Pattern[str]], ...] = (
    (
        "伝わっていないと感じた場面",
        re.compile(r"(最初に.*伝わっていない|どこで伝わっていない|伝え方のズレ|伝わりにくかった場面)", re.I),
    ),
    (
        "何を変えたか",
        re.compile(r"(次に.*(?:言い換え|変えた)|何を言い換えたか|何を変えたか|順番を変えた)", re.I),
    ),
    (
        "翌日に持ち越す学び",
        re.compile(r"(最後に.*(?:学び|持ち越す)|翌日へ持ち越す学び|翌日に持ち越す学び|日々の気づきと学び)", re.I),
    ),
)
_DAILY_STORY_SOURCE_SURFACE_LABELS: tuple[tuple[str, re.Pattern[str]], ...] = (
    (
        "伝わっていないと感じた場面",
        re.compile(r"(伝わっていなかった|伝わりにくかった|ズレ|同じ説明を二回)", re.I),
    ),
    (
        "何を変えたか",
        re.compile(r"(先に示した|短くしただけでは改善せず|一文目に置く|期限と必要な判断|順番を変え)", re.I),
    ),
)
...
_ALLOWED_SELF_REFERENCE_POLICIES = {"auto", "watashi", "watashitachi", "tousha", "heisha", "minimal"}
...
def _default_allowed_pronouns_for_contract(normalized: Dict[str, Any], defaults: Dict[str, Any]) -> List[str]:
    semantic_key = str(normalized.get("semantic_article_key") or normalized.get("article_type") or "").strip().lower()
    semantic_defaults = _SEMANTIC_ALLOWED_PRONOUN_DEFAULTS.get(semantic_key)
    if semantic_defaults:
        return list(semantic_defaults)
    return list(defaults.get("allowed_pronouns") or [])
...
def _is_fragmentary_source_fact(text: str) -> bool:
    value = _normalize_source_fact_text(text, max_length=120)
    if not value:
        return True
    if re.search(r"[。！？]$", value):
        return False
    if len(value) <= 28 and re.search(r"(?:が|けど|けれど|ものの|ため|ので|から|し|など|として|とは)$", value):
        return True
    return False
...
def _is_low_signal_source_fact(text: str) -> bool:
    ...
    if _SOURCE_META_PATTERN.search(value):
        return True
    if _is_fragmentary_source_fact(value):
        return True
    return False
...
_PRODUCT_INTRO_LABELLED_ANCHOR_RE = re.compile(
    r"(?:サービス名|商品名|製品名|提供サービス|名称|branded offering name)\s*[:：は]\s*"
    r"([A-Za-z0-9一-龥ぁ-んァ-ヴー・＋+&().（）/\- ]{2,48})",
    re.I,
)
_PRODUCT_INTRO_CATEGORY_ANCHOR_RE = re.compile(
    r"([A-Za-z0-9一-龥ぁ-んァ-ヴー・＋+&().（）/\- ]{0,36}?"
    r"(?:業務SaaS|[A-Za-z0-9][A-Za-z0-9._+\-]*\s*SaaS|支援サービス|整備支援サービス|"
    r"作成支援ツール|支援ツール|管理システム|管理ツール|作成ツール|業務ツール|業務システム|"
    r"プラットフォーム|ソフトウェア|ソリューション|パッケージ|サービス|ツール|製品|商品))"
    r"(?=$|[はをがのにでと、。,.:\s])",
    re.I,
)
_PRODUCT_INTRO_BARE_CATEGORY_ANCHOR_RE = re.compile(
    r"(業務SaaS|データ入力[・、/](?:スキャニング|チェック|納品)|データ入力・エントリ業務|"
    r"データ化支援|市場調査・Webリサーチ)",
    re.I,
)
_PRODUCT_INTRO_BRANDED_NAME_RE = re.compile(
    r"\b([A-Z][A-Za-z0-9][A-Za-z0-9._+\-]*(?:\s+[A-Z][A-Za-z0-9][A-Za-z0-9._+\-]*){0,3})\b"
)
_PRODUCT_INTRO_IGNORED_BRAND_TOKENS = {
    "AI",
    "API",
    "BTOB",
    "BTOC",
...
_PRODUCT_INTRO_GENERIC_ANCHOR_RE = re.compile(
    r"^(?:この|本|当該|同|当社の?|弊社の?|自社の?)?"
    r"(?:サービス|商品|製品|ツール|システム|アプリ|機能|支援|導入支援|初回設定|FAQ整備|"
    r"案内分岐|支援設計|機能の数|迷いを減らす|サービス概要|利用シーンメモ)$",
    re.I,
)


def _normalize_product_intro_subject_anchor_candidate(text: str) -> str:
    value = _normalize_source_fact_text(text, max_length=72).strip(" 　、。:：")
    if not value:
        return ""
    value = re.sub(
        r"^(?:サービス名|商品名|製品名|提供サービス|名称|branded offering name)\s*[:：は]\s*",
    ...
    value = re.sub(r"^(?:この|本|当該|同|当社の?|弊社の?|自社の?)\s*", "", value).strip(" 　、。:：")
    if not value or _PRODUCT_INTRO_GENERIC_ANCHOR_RE.fullmatch(value):
        return ""
    if re.fullmatch(r"(?:支援サービス|支援ツール|管理ツール|作成ツール)", value):
        return ""
    return value


def _extract_product_intro_subject_anchors_from_text(text: str) -> List[str]:
    value = _normalize_source_fact_text(text, max_length=320)
    if not value:
        return []
    anchors: List[str] = []
    for pattern in (
        _PRODUCT_INTRO_LABELLED_ANCHOR_RE,
        _PRODUCT_INTRO_CATEGORY_ANCHOR_RE,
        _PRODUCT_INTRO_BARE_CATEGORY_ANCHOR_RE,
    ):
        for match in pattern.finditer(value):
            anchor = _normalize_product_intro_subject_anchor_candidate(str(match.group(1) or ""))
            if anchor and anchor not in anchors:
                anchors.append(anchor)
    if re.search(r"(サービス|製品|商品|ツール|プラン|提供)", value):
        for match in _PRODUCT_INTRO_BRANDED_NAME_RE.finditer(value):
            anchor = _normalize_product_intro_subject_anchor_candidate(str(match.group(1) or ""))
            if not anchor or anchor.upper() in _PRODUCT_INTRO_IGNORED_BRAND_TOKENS:
                continue
...
def _assess_product_intro_subject_anchor(normalized: Dict[str, Any]) -> Dict[str, Any]:
    semantic_key = str(normalized.get("semantic_article_key") or normalized.get("article_type") or "").strip().lower()
    if semantic_key != "product_introduction":
        return {"is_sufficient": True, "anchors": []}
    anchors: List[str] = []
    for document in _normalize_source_documents(normalized.get("source_documents")):
        title = str(document.get("title") or "").strip()
        content = str(document.get("content") or "").strip()
        for text in (title, content):
            for anchor in _extract_product_intro_subject_anchors_from_text(text):
                if anchor not in anchors:
                    anchors.append(anchor)
                    if len(anchors) >= 3:
                        return {"is_sufficient": True, "anchors": anchors}
    return {"is_sufficient": bool(anchors), "anchors": anchors}
...
def _is_substantive_company_intro_fact(text: str, *, bucket: str) -> bool:
    value = _normalize_source_fact_text(text, max_length=180)
    if not value or _is_low_signal_source_fact(value):
        return False
    if len(value) >= 32:
        return True
    if re.search(r"(19|20)\d{2}年|\d+年|\d+社|\d+件|\d+人", value):
        return True
    if bucket == "overview":
        return bool(
            len(value) >= 18
            and re.search(r"(会社|当社|弊社|私たち|事業|サービス|業務|提供|支援|対応|開発|運営|製造)", value)
        )
    if bucket == "strength":
        return bool(
            len(value) >= 18
            and re.search(r"(強み|特長|特徴|体制|品質|実績|対応力|柔軟|ノウハウ|一貫|伴走|支える)", value)
        )
    if bucket == "history":
        return bool(re.search(r"(創業|設立|沿革|歩み|先代|代目)", value))
    return len(value) >= 24 and "。" in value


def _is_substantive_route_source_fact(text: str, *, semantic_key: str) -> bool:
    value = _normalize_source_fact_text(text, max_length=180)
    if not value or _is_low_signal_source_fact(value):
        return False
    if semantic_key == "announcement":
        return True
    if re.search(r"(19|20)\d{2}年|\d+月|\d+日|\d+%|\d+件|\d+社|\d+人", value):
        return True
    if semantic_key == "product_introduction":
        return _is_product_intro_service_fact(value) or bool(
            re.search(r"(製品|商品|サービス|機能|料金|プラン|仕様|用途|導入|運用|課題|判断材料)", value)
            and re.search(r"(できる|対応|支援|提供|使える|選ぶ|減らす|整える|改善)", value)
        )
    if semantic_key == "activity_introduction":
        return bool(re.search(r"(活動|取り組み|プロジェクト|研究|イベント|実施|開催|参加|支援)", value))
    if semantic_key == "recruit_culture":
        return bool(re.search(r"(採用|募集|働き方|チーム|社員|制度|カルチャー|職種|環境)", value))
    if semantic_key == "implementation_case":
        return bool(re.search(r"(導入前|導入後|導入|オンボーディング|移行|活用|進め方|課題|変化)", value))
    if semantic_key == "improvement_case":
        return bool(re.search(r"(改善前|改善後|改善|見直し|最適化|効率化|変化|課題|結果)", value))
    if semantic_key == "incident_case":
        return bool(re.search(r"(障害|事故|インシデント|不具合|原因|影響|再発防止|対応)", value))
    if semantic_key == "learning_case":
        return bool(re.search(r"(ケース|学び|振り返り|教材|前提|判断|次に生かす|lesson)", value, re.I))
    if semantic_key == "comparative_review":
        if re.search(r"(比較|違い|差分|他社|競合|条件|用途|結論|vs)", value, re.I):
            return True
        if re.search(
            r"(?:評価軸|比較軸|観点|選び分け|向く条件|注意点|確認順|次の確認|話者分離|保存先|共有方法|保存期間)",
            value,
            re.I,
        ):
            return True
        price_or_plan = bool(
            re.search(
                r"(?:価格|料金|費用|月額|年額|無料|有料|プラン|[0-9０-９]+(?:[.,．][0-9０-９]+)?\s*(?:円|万円|ドル|USD))",
                value,
                re.I,
            )
        )
        fit_or_use = bool(
            re.search(
                r"(?:向け|合う|適する|用途|規模|チーム|部門|組織|運用|導入|定着|承認|権限|監査|サポート|重視)",
                value,
            )
        )
        option_detail = bool(
            re.search(r"(?:特徴|中心|提供|多い|弱い|強い|柔軟|厳格|専任|初期設定|テンプレート|CS)", value, re.I)
        )
        return bool((price_or_plan and fit_or_use) or (fit_or_use and option_detail))
    return len(value) >= 24 and "。" in value


def _assess_comparative_source_substance(normalized: Dict[str, Any]) -> Dict[str, Any]:
    documents = _normalize_source_documents(normalized.get("source_documents"))
    source_texts = [
        _normalize_source_fact_text(str(document.get("content") or ""), max_length=1200)
        for document in documents
    ]
    ...
    option_patterns = (
        r"\b([A-Z][A-Za-z0-9_-]{0,20})\s*は",
        r"\b(Tool\s*[A-Z])\b",
        r"\b(Product\s*[A-Z])\b",
        r"\b(Service\s*[A-Z])\b",
        r"(?:候補|案)\s*([A-ZＡ-Ｚ0-9０-９一二三])",
    )
    ignored_tokens = {"AI", "PDF", "CSV", "FAQ", "URL", "CS", "Markdown"}
    for pattern in option_patterns:
        for match in re.finditer(pattern, combined, re.I):
            token = re.sub(r"\s+", "", str(match.group(1) or "")).strip()
            if token and token not in ignored_tokens:
                option_tokens.add(token.lower())
    comparison_target_match = re.search(
        r"比較対象は(.{0,80}?)(?:の[23２３三二]つ|[23２３三二]つ|。)",
        combined,
        re.I,
    )
    if comparison_target_match:
        for token in re.split(r"[、,／/・\s]+", comparison_target_match.group(1)):
            compact = re.sub(r"[^A-Za-z0-9一-龥ぁ-んァ-ヴー]+", "", token)
            if compact and len(compact) >= 1 and compact.lower() not in ignored_tokens:
                option_tokens.add(compact.lower())

    axis_labels = _resolve_comparison_axes(normalized)
    axis_hit_count = 0
    for axis in axis_labels:
        axis_text = str(axis or "").strip()
        if axis_text and axis_text in combined:
            axis_hit_count += 1
    axis_support_hits = 0
    if any(axis in {"価格", "料金", "費用"} for axis in axis_labels) and re.search(
        r"(?:価格|料金|費用|月額|年額|無料|有料|円|万円)",
        combined,
    ):
        axis_support_hits += 1
    if any(axis in {"用途", "向く条件"} for axis in axis_labels) and re.search(
        r"(?:用途|向け|向く|合う|適する|規模|チーム|部門|組織|運用)",
        combined,
    ):
        axis_support_hits += 1
    if any(axis in {"承認フロー", "レビューの流れ"} for axis in axis_labels) and re.search(
        r"(?:承認|レビュー|差し戻し|権限|履歴)",
        combined,
    ):
        axis_support_hits += 1
    if any(axis in {"サポート", "サポート密度 / 導入支援", "導入支援"} for axis in axis_labels) and re.search(
        r"(?:サポート|支援|CS|導入支援|テンプレート)",
        combined,
    ):
        axis_support_hits += 1
    ...
    buckets = {
        "comparison_context": bool(
            len(option_tokens) >= 2
            or re.search(r"(?:比較対象|比較条件|候補|[23２３三二]案|[23２３三二]つ)", combined)
        ),
        "evaluation_axes": bool(
            axis_support_hits >= 2
            or (axis_hit_count >= 1 and axis_support_hits >= 1)
            or axis_hit_count >= 2
            or re.search(r"(?:共通評価軸|評価軸|比較軸|観点|話者分離|保存先|共有方法|保存期間)", combined)
        ),
        "option_differences": bool(
            len(option_tokens) >= 2
            and re.search(
                r"(?:違い|差分|一方|弱い|強い|自動|手動|不可|できる|前提|中心|出力|保存|共有|提供|対応|依存)",
                combined,
            )
        ),
        "fit_conditions": bool(
            re.search(r"(?:向く条件|向く|向け|合う|適する|選び分け|条件|場合|重視|優先|前提)", combined)
        ),
        "tradeoffs_or_cautions": bool(
            re.search(
                r"(?:注意点|注意|留意|必要|弱い|不可|依存|事前|同意|設計|未確認|書かない|"
                r"中心|厳格|別料金|手動|権限管理|監査要件|初期設定|保存期間|端末購入)",
                combined,
            )
        ),
        "decision_next_step": bool(
            re.search(r"(?:確認順|次の確認|確認できる順|最初に|次に|最後に|先に|順)", combined)
        ),
    }
    if not buckets["decision_next_step"] and all(
        buckets.get(key)
        for key in ("evaluation_axes", "fit_conditions", "tradeoffs_or_cautions")
    ):
        buckets["decision_next_step"] = True

    substantive_buckets = [key for key, present in buckets.items() if present]
    is_sufficient = bool(
        len(documents) >= 2
    ...
    return {
        "substantive_item_count": len(substantive_buckets),
        "substantive_buckets": substantive_buckets,
        "option_count": len(option_tokens),
        "is_sufficient": is_sufficient,
    }


def _assess_company_intro_source_substance(normalized: Dict[str, Any]) -> Dict[str, Any]:
    if not _is_company_introduction_like(normalized):
        return {
            "substantive_item_count": 0,
            "substantive_char_total": 0,
            "substantive_buckets": [],
            "is_sufficient": True,
        }
    substantive_buckets: List[str] = []
    substantive_item_count = 0
    substantive_char_total = 0
    for item in list(normalized.get("source_grounding_items") or []):
        ...
        if not _is_substantive_company_intro_fact(fact_text, bucket=bucket):
            continue
        substantive_item_count += 1
        substantive_char_total += len(_normalize_source_fact_text(fact_text, max_length=180))
        if bucket not in substantive_buckets:
            substantive_buckets.append(bucket)
    return {
        "substantive_item_count": substantive_item_count,
        "substantive_char_total": substantive_char_total,
        "substantive_buckets": substantive_buckets,
        "is_sufficient": substantive_item_count >= 2,
    }


def _assess_route_source_substance(normalized: Dict[str, Any]) -> Dict[str, Any]:
    semantic_key = str(normalized.get("semantic_article_key") or normalized.get("article_type") or "").strip().lower()
    if semantic_key in {"", "announcement", "company_introduction"}:
        return {
            "substantive_item_count": 0,
            "is_sufficient": True,
        }
    if semantic_key == "comparative_review":
        return _assess_comparative_source_substance(normalized)
    substantive_item_count = 0
    for item in list(normalized.get("source_grounding_items") or []):
        if not isinstance(item, dict):
            continue
        if _is_substantive_route_source_fact(str(item.get("fact_text") or ""), semantic_key=semantic_key):
            substantive_item_count += 1
    required_count = 2 if semantic_key == "comparative_review" else 1
    return {
        "substantive_item_count": substantive_item_count,
        "is_sufficient": substantive_item_count >= required_count,
    }
...
def _derive_source_grounding_status(normalized: Dict[str, Any]) -> str:
    ...
    if _is_company_introduction_like(normalized):
        substance = _assess_company_intro_source_substance(normalized)
        if not bool(substance.get("is_sufficient")):
            return "insufficient"
    else:
        substance = _assess_route_source_substance(normalized)
        if not bool(substance.get("is_sufficient")):
            return "insufficient"
...
def _build_source_fit(normalized: Dict[str, Any]) -> Dict[str, Any]:
    ...
    status = "pass"
    company_intro_substance = _assess_company_intro_source_substance(normalized)
    route_source_substance = _assess_route_source_substance(normalized)
    product_subject_anchor = _assess_product_intro_subject_anchor(normalized)
    ...
    if not documents and not bool(normalized.get("source_grounding_required")):
        return {
            ...
            "summary": "材料の読み取りは、URL だけの段階ではまだ確定しません。",
        }
    if bool(normalized.get("source_grounding_required")) and not documents:
        ...
        if not bool(company_intro_substance.get("is_sufficient")):
            status = "block"
            missing_buckets.append("source_substance")
            required_actions.append("会社概要・強み・沿革が分かる本文量のある公式ソースを2件以上そろえてください。")
        ...
        elif not bool(route_source_substance.get("is_sufficient")):
            status = "block"
            missing_buckets.append("source_substance")
            required_actions.append("商品・サービスの内容や用途が分かる具体的なソースを追加してください。")
        elif not bool(product_subject_anchor.get("is_sufficient")):
            status = "block"
            missing_buckets.append("product_subject_anchor")
            required_actions.append("商品名・サービス名・明確なサービスカテゴリが分かるソースを追加してください。")
        ...
        if int(signal_scores.get("activity", 0)) <= 0:
            ...
            status = "block"
            missing_buckets.append("source_substance")
            required_actions.append("活動内容や取り組みの実態が分かる具体的なソースを追加してください。")
        ...
        if int(signal_scores.get("recruit", 0)) <= 0:
            ...
            status = "block"
            missing_buckets.append("source_substance")
            required_actions.append("働き方やチームの実態が分かる具体的なソースを追加してください。")
        ...
        if int(signal_scores.get("implementation", 0)) <= 0:
            ...
            status = "block"
            missing_buckets.append("source_substance")
            required_actions.append("導入前後や進め方が分かる具体的な事例ソースを追加してください。")
        ...
        if int(signal_scores.get("improvement", 0)) <= 0:
            ...
            status = "block"
            missing_buckets.append("source_substance")
            required_actions.append("改善前後の変化が分かる具体的なソースを追加してください。")
        ...
        if int(signal_scores.get("incident", 0)) <= 0:
            ...
            status = "block"
            missing_buckets.append("source_substance")
            required_actions.append("障害内容、影響、原因、対応が分かる具体的なソースを追加してください。")
        ...
        if int(signal_scores.get("learning", 0)) <= 0 and int(signal_scores.get("implementation", 0)) <= 0:
            ...
            status = "block"
            missing_buckets.append("source_substance")
            required_actions.append("ケースの前提や学びが分かる具体的なソースを追加してください。")
        ...
        elif documents and not bool(route_source_substance.get("is_sufficient")):
            status = "block"
            missing_buckets.append("source_substance")
            required_actions.append("比較対象の条件や差分が分かる具体的なソースを2件以上そろえてください。")
        elif len(documents) < 2:
            status = "warn"
            required_actions.append("比較対象の差分が分かるソースを2件以上そろえると安定します。")

    summary_map = {
        "pass": "材料はこの内容にだいたい合っています。",
        "warn": "このままでも進められますが、補足資料があると安定します。",
        "block": "このままでは内容を確定しにくいため、材料の追加が必要です。",
    }
    return {
        "status": status,
        ...
        "candidate_targets": candidate_targets,
        "matched_signals": signal_scores,
        "product_subject_anchor": dict(product_subject_anchor) if semantic_key == "product_introduction" else {},
...
def _derive_blank_prompt_company_intro_topic_statement(normalized: Dict[str, Any]) -> str:
    if not _is_company_introduction_like(normalized):
        return ""
    if str(normalized.get("prompt_raw") or normalized.get("topic") or "").strip():
        return ""
    ...
    topic_source_items = _build_source_grounding_items(normalized)
    if not topic_source_items:
        return ""
    topic_normalized = dict(normalized)
    topic_normalized["source_grounding_items"] = topic_source_items
    source_backed_surface_items = _derive_source_backed_company_intro_surface_labels(
        topic_normalized,
        limit=2,
    )
    has_current_business = any(
        str(item.get("bucket") or "").strip() in {"overview", "strength"}
        for item in topic_source_items
    )
    if not has_current_business:
        return ""
    has_strength = any(
        str(item.get("bucket") or "").strip() == "strength"
        for item in topic_source_items
    )
    has_history = any(
        str(item.get("bucket") or "").strip() == "history"
        for item in topic_source_items
    )
    if has_history and not has_strength:
        return "自社の事業内容と歩みを紹介する"
    if "選ばれる理由" in source_backed_surface_items:
        return "自社の事業内容と選ばれる理由を紹介する"
    if has_history:
        return "自社の事業内容と歩みを紹介する"
    return "自社の事業内容を紹介する"


def _derive_blank_prompt_company_intro_current_focus(normalized: Dict[str, Any]) -> str:
    if not _is_blank_prompt_company_intro(normalized):
        return ""
    source_grounding_items = _build_source_grounding_items(normalized)
    current_blob = " ".join(
        _normalize_source_fact_text(item.get("fact_text", ""), max_length=120)
        for item in source_grounding_items
        if str(item.get("bucket") or "").strip() in {"overview", "strength"}
    )
    if not current_blob:
        return ""
    if re.search(
        r"(データ入力|データエントリー|入力).*?(スキャニング|スキャン|入力・チェック|ベリファイ|納品)|"
        r"(スキャニング).*(データ入力|データエントリー)",
        current_blob,
        re.I,
    ):
        return "データ入力とスキャニング"
    if re.search(r"(データ入力|データエントリー)", current_blob, re.I):
        return "データ入力"
    if re.search(r"(エントリー入力|ベリファイ入力|入力).*?(チェック|納品|確認)", current_blob, re.I):
        return "データ入力"
    if re.search(r"(事業|サービス|業務)", current_blob, re.I):
        return "事業内容"
    if current_blob:
        return "事業内容"
    return ""
...
def _derive_blank_prompt_company_intro_background_clause(normalized: Dict[str, Any]) -> str:
    if not _is_blank_prompt_company_intro(normalized):
        return ""
    source_grounding_items = _build_source_grounding_items(normalized)
    history_blob = " ".join(
        _normalize_source_fact_text(item.get("fact_text", ""), max_length=120)
        for item in source_grounding_items
        if str(item.get("bucket") or "").strip() == "history"
    )
    if not history_blob:
        return ""
    if re.search(r"(創業|歴史|歩み|業務転換|1885|1968|2019)", history_blob, re.I):
        return "今の仕事につながる背景"
    return "背景"


def _is_blank_prompt_company_intro(normalized: Dict[str, Any]) -> bool:
    if not _is_company_introduction_like(normalized):
        return False
    return not str(normalized.get("prompt_raw") or normalized.get("topic") or "").strip()


def _derive_company_intro_background_label(
    normalized: Dict[str, Any],
    bucket_items: List[str],
    *,
    source_backed_surface_items: List[str] | None = None,
) -> str:
    if "歩み" not in bucket_items:
        return ""
    if not _is_blank_prompt_company_intro(normalized):
        return "歩み"
    source_backed_surface_items = [str(item or "").strip() for item in list(source_backed_surface_items or []) if str(item or "").strip()]
    has_current_point = any(item in {"事業内容", "強み"} for item in bucket_items) or bool(source_backed_surface_items)
...
def _derive_prompt_daily_story_surface_labels(normalized: Dict[str, Any]) -> List[str]:
    semantic_key = str(normalized.get("semantic_article_key") or normalized.get("article_type") or "").strip().lower()
    ...
    if not prompt_raw:
        return []
    labels: List[str] = []
    for label, pattern in _DAILY_STORY_PROMPT_SURFACE_LABELS:
        if pattern.search(prompt_raw):
            labels.append(label)
    return _dedupe_text_list(labels, limit=3)


def _derive_source_backed_daily_story_surface_labels(
    normalized: Dict[str, Any],
    *,
    limit: int = 2,
) -> List[str]:
    ...
    if not source_blob:
        return []
    labels: List[str] = []
    for label, pattern in _DAILY_STORY_SOURCE_SURFACE_LABELS:
        if pattern.search(source_blob):
            labels.append(label)
    return _dedupe_text_list(labels, limit=limit)


def _derive_daily_story_reader_surface_items(
    normalized: Dict[str, Any],
    *,
    limit: int = 3,
) -> List[str]:
    ...
    if semantic_key != "daily_story":
        return []
    prompt_surface_items = _derive_prompt_daily_story_surface_labels(normalized)
    source_surface_items = _derive_source_backed_daily_story_surface_labels(normalized, limit=2)
    prompt_raw = str(normalized.get("prompt_raw") or normalized.get("topic") or "").strip()

    prioritized: List[str] = []
    if "伝わっていないと感じた場面" in prompt_surface_items or "伝わっていないと感じた場面" in source_surface_items:
        prioritized.append("伝わっていないと感じた場面")
    if "何を変えたか" in prompt_surface_items or "何を変えたか" in source_surface_items:
        prioritized.append("何を変えたか")
    if "翌日に持ち越す学び" in prompt_surface_items:
        prioritized.append("翌日に持ち越す学び")
    elif re.search(r"(学び|気づき)", prompt_raw):
        prioritized.append("翌日に持ち越す学び")

    return _dedupe_text_list(
        [*prioritized, *prompt_surface_items, *source_surface_items],
        limit=limit,
    )
...
def _derive_company_intro_reader_surface_items(
    ...
    *,
    ...
    background_label = _derive_company_intro_background_label(
        normalized,
        bucket_items,
        source_backed_surface_items=source_backed_surface_items,
    )
    ...
    if not requested_surfaces:
        ...
        if "大切にしている姿勢" in source_backed_surface_items and "強み" not in bucket_items:
            prioritized.append("大切にしている姿勢")
        elif "選ばれる理由" in source_backed_surface_items:
            prioritized.append("選ばれる理由")
        elif "強み" in bucket_items:
            prioritized.append("強み")
        prioritized.extend([item for item in source_backed_surface_items if item not in prioritized])
        return _dedupe_text_list(
            [
                *prioritized,
                *(
                    [background_label]
                    if background_label and background_label not in prioritized
                    else []
                ),
                *[
                    item
                    ...
                    if item != "歩み"
                    and not (item == "強み" and "選ばれる理由" in prioritized)
                ],
                *supplemental_items,
            ],
            limit=limit,
    ...
    if background_label:
        prioritized.append(background_label)
    ...
    for item in [*bucket_items, *source_backed_surface_items, *prompt_surface_items, *supplemental_items]:
        if item == "強み" and "選ばれる理由" in prioritized:
            continue
        if item == "歩み" and background_label and background_label != "歩み":
            continue
...
def _derive_prompt_surface_items(normalized: Dict[str, Any]) -> Dict[str, Any]:
    ...
    raw_items.extend(_derive_prompt_daily_story_surface_labels(normalized))
...
def _derive_focus_bundle(normalized: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if prompt_primary:
        ...
        if semantic_key == "daily_story":
            support_points.extend(_derive_daily_story_reader_surface_items(normalized, limit=2))
...
def _derive_must_cover(normalized: Dict[str, Any]) -> List[str]:
    ...
    if semantic_key == "daily_story":
        merged_items = _derive_daily_story_reader_surface_items(normalized, limit=3)
        if merged_items:
            return merged_items
...
def _resolve_topic_statement(
    ...
    *,
    ...
    if article_type == "branding" and semantic_key == "company_introduction":
        source_backed_topic = _derive_blank_prompt_company_intro_topic_statement(normalized)
        if source_backed_topic:
            return source_backed_topic, "source_grounding"
...
def resolve_input_contract(payload: Dict[str, Any]) -> ContractResolveResult:
    ...
    if str(normalized.get("source_mode") or "").strip().lower() == PROMPT_ONLY_SOURCE_MODE and prompt_only_has_source_conflict(normalized):
        raise InputContractValidationError(
            "prompt_only cannot be combined with source documents",
            reason_code="INP_PROMPT_ONLY_SOURCE_CONFLICT",
            field="source_documents",
        )
    normalized["past_blog_context"] = normalize_past_blog_context(
        normalized.get("past_blog_context"),
        allow_factual_carry=bool(
            str(normalized.get("source_mode") or "").strip().lower() == "grounded"
            and normalized.get("source_documents")
        ),
    )
    ...
    normalized["self_reference_policy"] = _normalize_choice(
        normalized.get("self_reference_policy"),
        "auto",
        _ALLOWED_SELF_REFERENCE_POLICIES,
    ...
    explicit_allowed_pronouns = _normalize_string_list(normalized.get("allowed_pronouns"))
    if explicit_allowed_pronouns:
        normalized["allowed_pronouns"] = explicit_allowed_pronouns[:4]
        field_sources["allowed_pronouns"] = _field_source_or_default(
            field_sources,
            "allowed_pronouns",
            "input_contract",
        )
    else:
        normalized["allowed_pronouns"] = _default_allowed_pronouns_for_contract(normalized, defaults)
        if normalized["allowed_pronouns"]:
            field_sources["allowed_pronouns"] = _field_source_or_default(
                field_sources,
                "allowed_pronouns",
                "semantic_article_key_default",
            )
        else:
            field_sources.pop("allowed_pronouns", None)
